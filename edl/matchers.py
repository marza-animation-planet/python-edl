...
class CommentMatcher(Matcher):
    ...
    def apply(self, edl, line):
        ...
        if m:
            ...
            if len(edl) > 0:
                edl[-1].comments.append("* " + m.group(1))
                mo = re.search('\*\s+FROM\s+CLIP\s+NAME:\s+(.+)', line)
                if mo:
                    edl[-1].clip_name = mo.group(1).strip()
                return True
        else:
            return False
...
class NameMatcher(Matcher):
    ...
    def apply(self, edl, line):
        m = re.search(self.regex, line)
        if m and len(edl) > 0:
            edl[-1].clip_name = m.group(2).strip()
            return True
        else:
            return False
...
class SourceMatcher(Matcher):
    ...
    def apply(self, edl, line):
        ...
        if m and len(edl) > 0:
            edl[-1].source_file = m.group(2).strip()
            return True
        else:
            return False
...
class EffectMatcher(Matcher):
    ...
    def apply(self, edl, line):
        m = re.search(self.regex, line)
        if m:
            edl[-1].transition.effect = m.group(2).strip()
            return True
        else:
            return False
